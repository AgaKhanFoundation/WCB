...
describe('achievement', () => {
  context('GET /achievement', () => {
    it('should return achievements', async () => {
      let a1 = await models.db.achievement.create({sequence: 1, name: 'London', distance: 100, description: 'AKF UK', flag_name: 'greatbritain', map_image: 'challenge1_0.png', title: 'Article1', subtitle: 'Author1', content: 'content1 with html tags', media: 'photo:photo1_url video:video1_url'})
      let a2 = await models.db.achievement.create({sequence: 2, name: 'Paris', distance: 200, description: 'AKF Paris', flag_name: 'france', map_image: 'challenge1_1.png', title: 'Article2', subtitle: 'Author2', content: 'content2 with html tags', media: 'photo:photo2_url video:video2_url'})
      await koaRequest
        .get('/achievement')
        .expect(200)
        .then(response => {
  ...
  context('POST /achievement', () => {
    it('should create achievement with sequence=sequence, name=name, distance=distance, description=description, flag_name=flag_name, map_image=map_image, title=title, subtitle=subtitle, content=subtitle, and media=media', async () => {
      let sequence = 1
      let name = 'London'
      let distance = 100
      let description = ''
      ...
      await koaRequest
        .post('/achievement')
        .send({sequence, name, distance, description, flag_name: flagName, map_image: mapImage, title, subtitle, content, media})
        .expect(201)
        .then(response => {
  ...
  context('PATCH /achievement/:id', () => {
    it('should change achievement sequence, name, distance, description, flag_name, map_image, title, subtitle, content, and media', async () => {
      let achievement = await models.db.achievement.create({sequence: 1, name: 'London', distance: 100, description: 'AKF UK', flag_name: 'greatbritain', map_image: 'challenge1_0.png', title: 'Article1', subtitle: 'Author1', content: 'content1 with html tags', media: 'photo:photo1_url video:video1_url'})
      await koaRequest
        .patch('/achievement/' + achievement.id)
        .send({sequence: 2, name: 'Paris', distance: 200, description: 'AKF Paris', flag_name: 'france', map_image: 'challenge1_1.png', title: 'Article2', subtitle: 'Author2', content: 'content2 with html tags', media: 'photo:photo2_url video:video2_url'})
        .expect(200, [1])
    })
