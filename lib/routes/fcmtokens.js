...
router
  ...
  .post('/fcmtokens', async (ctx) => {
    ...
    // if fcmtoken is to be associated with an fbid ensure that fbid is valid
    if (fbid) {
      ...
      if (participant) {
        ...
        // ensure that an fcmtoken is not already assigned to this participant
      ...
      await models.db.fcmtoken
        .findOrCreate({where: {fcm_token: fcmTokenVal}})
        .spread(function (fcmtoken, created) {
          if (created) {
            ctx.status = 201
            ctx.body = fcmtoken
