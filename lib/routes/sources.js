...
router
  .get('/sources', async (ctx) => {
    ctx.body = await models.db.source.findAll({order: ['name', 'ASC']})
  })
  .get('/sources/:id', async (ctx) => {
    ctx.body = await models.db.source.findById(ctx.params.id)
  })
  .post('/sources', async (ctx) => {
    let name = ctx.request.body.name
    await models.db.source
  ...
  .del('/sources/:id', async (ctx) => {
    await models.db.source.destroy({
      where: {id: ctx.params.id}
    })
      .then(result => {
