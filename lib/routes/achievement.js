...
router
  .get('/achievement', async (ctx) => {
    ctx.body = await models.db.achievement.findAll({order: 'distance ASC'})
  })
  .get('/achievement/:id', async (ctx) => {
    ctx.body = await models.db.achievement.findById(ctx.params.id)
  })
    ...
    await models.db.achievement
      ...
      .spread(function (achievement, created) {
        if (created) {
          ...
          ctx.status = 409
          ctx.body = `Achievement with name="${name}" already exists!`
        }
      })
  })
