...
router
  ...
  .post('/achievement', async (ctx) => {
    ...
    // Verify name is not used to comply with uniqueness constraint
    let achievement = await models.db.achievement.findOne({where: {name}})
    if (achievement) {
      ctx.status = 409
      ctx.body = {'error': {
        'code': 409,
      ...
      await models.db.achievement
        .findOrCreate({where: {sequence, name, distance, title, subtitle, description, media}})
