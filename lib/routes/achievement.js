...
router
  ...
  .post('/achievement', async (ctx) => {
    ...
    let flagName = ctx.request.body.flag_name
    ...
    if (achievement) {
      ...
      await models.db.achievement
        .findOrCreate({where: {sequence: sequence, name: name, distance: distance, description: description, flag_name: flagName, map_image: mapImage, title: title, subtitle: subtitle, content: content, media: media}})
