...
router
  ...
  .patch('/participants/:fbid', async (ctx) => {
    ...
    let changes = {}
    if (body.fbid) { changes.fbid = body.fbid }
    if (body.team_id) { changes.team_id = JSON.parse(body.team_id) }
    if (body.source_id) { changes.source_id = JSON.parse(body.source_id) }
    if (body.cause_id) { changes.cause_id = JSON.parse(body.cause_id) }
