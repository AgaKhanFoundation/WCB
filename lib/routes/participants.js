...
router
  ...
  .get('/participants/:fbid/social', async (ctx) => {
    const user = await admin.auth().getUser(ctx.params.fbid);
    if (user) {
      ctx.body = {
  ...
  .post('/participants', async (ctx) => {
    ...
    let registered = ctx.request.body.registered
    // Verify fbid is not used to comply with uniqueness constraint
    let participant = await models.db.participant.findOne({where: {fbid}})
    if (participant) {
      ctx.status = 409
      ctx.body = {'error': {
        'code': 409,
      ...
      await models.db.participant
        .findOrCreate({where: {fbid, registered}})
